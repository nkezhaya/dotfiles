function __bobthefish_colors -S -a color_scheme -d 'Define colors used by bobthefish'
  switch "$color_scheme"
    ...
    case 'catppuccin'
      set -l cat_rosewater f5e0dc
      set -l cat_flamingo  f2cdcd
      set -l cat_pink      f5c2e7
      set -l cat_mauve     cba6f7
      set -l cat_red       f38ba8
      set -l cat_maroon    eba0ac
      set -l cat_peach     fab387
      set -l cat_yellow    f9e2af
      set -l cat_green     a6e3a1
      set -l cat_teal      94e2d5
      set -l cat_sky       89dceb
      set -l cat_sapphire  74c7ec
      set -l cat_blue      89b4fa
      set -l cat_lavender  b4befe
      set -l cat_text      cdd6f4
      set -l cat_subtext1  bac2de
      set -l cat_subtext0  a6adc8
      set -l cat_overlay2  9399b2
      set -l cat_overlay1  7f849c
      set -l cat_overlay0  6c7086
      set -l cat_surface2  585b70
      set -l cat_surface1  45475a
      set -l cat_surface0  313244
      set -l cat_base      1e1e2e
      set -l cat_mantle    181825
      set -l cat_crust     11111b

      set -l colorfg $cat_mantle
      set -l colorfg2 $cat_text

      set -x color_initial_segment_exit     $cat_surface2 $cat_red --bold
      set -x color_initial_segment_private  $cat_surface2 $cat_flamingo
      set -x color_initial_segment_su       $cat_surface2 $cat_green --bold
      set -x color_initial_segment_jobs     $cat_surface2 $cat_peach --bold

      set -x color_path                     $cat_surface0 $colorfg2
      set -x color_path_basename            $cat_surface0 $colorfg2 --bold
      set -x color_path_nowrite             $cat_surface0 $cat_mauve
      set -x color_path_nowrite_basename    $cat_surface0 $cat_mauve --bold

      set -x color_repo                     $cat_green $colorfg
      set -x color_repo_work_tree           $cat_surface2 $colorfg --bold
      set -x color_repo_dirty               $cat_red $colorfg
      set -x color_repo_staged              $cat_yellow $colorfg

      set -x color_vi_mode_default          $cat_sky $colorfg --bold
      set -x color_vi_mode_insert           $cat_green $colorfg --bold
      set -x color_vi_mode_visual           $cat_mauve $colorfg --bold

      set -x color_vagrant                  $cat_surface2 $colorfg2 --bold
      set -x color_k8s                      $cat_surface2 $colorfg2 --bold
      set -x color_aws_vault                $cat_yellow $colorfg --bold
      set -x color_aws_vault_expired        $cat_red $colorfg --bold
      set -x color_username                 $cat_surface2 $cat_yellow --bold
      set -x color_hostname                 $cat_surface2 $cat_yellow
      set -x color_rvm                      $cat_teal $colorfg --bold
      set -x color_nvm                      $cat_teal $colorfg --bold
      set -x color_virtualfish              $cat_teal $colorfg --bold
      set -x color_virtualgo                $cat_teal $colorfg --bold
      set -x color_desk                     $cat_teal $colorfg --bold
